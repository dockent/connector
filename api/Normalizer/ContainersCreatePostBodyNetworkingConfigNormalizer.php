...
class ContainersCreatePostBodyNetworkingConfigNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        if ($type !== 'Dockent\\OpenAPI\\Model\\ContainersCreatePostBodyNetworkingConfig') {
            return false;
        }
        return true;
    }
    ...
    {
        if ($data instanceof \Dockent\OpenAPI\Model\ContainersCreatePostBodyNetworkingConfig) {
            return true;
        }
        return false;
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \Dockent\OpenAPI\Model\ContainersCreatePostBodyNetworkingConfig();
        if (property_exists($data, 'EndpointsConfig')) {
            $values = new \ArrayObject(array(), \ArrayObject::ARRAY_AS_PROPS);
            foreach ($data->{'EndpointsConfig'} as $key => $value) {
                $values[$key] = $this->denormalizer->denormalize($value, 'Dockent\\OpenAPI\\Model\\EndpointSettings', 'json', $context);
            }
